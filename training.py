...
class NetworkTrainer():
    ...
    def training(self):
        ...
        def get_Qs(observation, hat=False):
        ...
        def temp_agent_hat(observation, _):
            return int(np.argmax(np.array([(q.item() if observation['board'][c] == 0 else -float('inf')) for c, q in enumerate(get_Qs(observation, hat=True))])))
        ...
        for episode in range(self.episodes):
            ...
            if episode % 2 == 0:
                trainer = self.env.train([None, temp_agent_hat])
                player_pos = 1
            else:
                trainer = self.env.train([temp_agent_hat, None])
                player_pos = 2
